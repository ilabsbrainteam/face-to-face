...
We only run this script with aparc_sub parcellation (not our custom ones),
because aparc_sub is the one used in 10.1016/j.neuroimage.2018.02.018, so it's
all we need to compare/verify that our choice of threshold is a reasonable one.
...
skips = load_params(os.path.join(param_dir, 'skip_labels.yaml'))[parcellation]
